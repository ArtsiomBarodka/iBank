 ...
 */
public class UserMonth implements Command {
    @Override
    public String execute(String request) {
        ...
        try {
            ...
            List<Category> categoryList = userService.getAllCategories(userId);
            User user = userService.getUserById(userId);

            if (categoryList == null){
                response = "За месяц не было трат";
            } else {
                StringBuilder result = new StringBuilder();

                result.append("Траты за месяц:\n");

                for (Category c:categoryList) {
                    result.append(c.getName()).append(": ")
                            .append(c.getMonthExpense()).append(";\n");
                }
